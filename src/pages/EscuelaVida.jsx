...
const EscuelaDeVida = () => {
    ...
    const {createInsEsc} = useInscriptionsStore();

    const handleAddInscription = async () => {
        const {success, message} = await createInsEsc(newInscription);
        if(!success) {
            toast({
                title:"Error",
                description: message,
    ...
    <VStack spacing={8}>
        ...
        <Heading as={"h1"} textAlign={"center"} mb={8} size={'2xl'}>
            Inscripción a la Escuela de Vida
        </Heading>
